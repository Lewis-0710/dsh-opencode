...
if [ -s "$TMP_DIR/sync.patch" ]; then
    ...
    if git apply --check "$TMP_DIR/sync.patch" 2>/dev/null; then
        ...
        git add -A
        git commit -m "sync: 与 upstream/master 同步 ($(date +%Y-%m-%d))" --quiet || true
...
if [ "$APPLIED_VIA_PATCH" = false ]; then
    ...
    if [ -z "$CONFLICTS" ]; then
        git add -A
        git commit -m "merge: 合并 upstream/master ($(date +%Y-%m-%d))" --quiet 2>/dev/null || true
    else
        ...
        git add -A
        git commit -m "merge: 智能合并 upstream/master ($(date +%Y-%m-%d))" --quiet 2>/dev/null || true
        echo "   ✅ Smart Merge 完成"
    fi
fi
