...
if ! git remote get-url upstream >/dev/null 2>&1; then
    ...
    echo "💡 请先运行: git remote add upstream https://github.com/FishBottle7/opencode2dsh.git"
    exit 1
fi
...
# Step 0: 创建安全临时备份目录
TMP_DIR="$(mktemp -d /tmp/dsh_opencode_sync_XXXXXX)"
cleanup() {
    rm -rf "$TMP_DIR"
}
...
# Step 2: 重置 master 分支到 upstream/master
echo "[2/4] 更新 local master 分支到 upstream/master..."
...
if [ "$APPLIED_VIA_PATCH" = false ]; then
    ...
    echo "   执行 Smart Merge..."
    git merge master --no-commit --no-ff 2>/dev/null || true
    CONFLICTS=$(git diff --name-only --diff-filter=U 2>/dev/null || true)

    if [ -z "$CONFLICTS" ]; then
        git add -A
        git commit -m "merge: sync upstream/master $(date +%Y-%m-%d)" --quiet 2>/dev/null || true
    else
        ...
        git add -A
        git commit -m "merge: sync upstream/master (smart merge, $(date +%Y-%m-%d))" --quiet 2>/dev/null || true
        echo "   ✅ Smart Merge 完成"
    fi
fi
...
git diff upstream/master...custom ':!sync.patch' ':!sync.sh' > "$PATCH_FILE"
...
if [ "$SKIP_TEST" = false ]; then
    if command -v pnpm >/dev/null 2>&1; then
        echo ""
        echo "🔍 正在运行项目构建与类型检查..."
        cd "$SCRIPT_DIR/packages/plugin"
        if pnpm build && pnpm typecheck; then
            echo "✅ 编译与类型检查全部通过！"
        else
            echo "❌ 警告: 编译或类型检查失败，请检查代码！"
        fi
        cd "$SCRIPT_DIR"
    fi
fi
